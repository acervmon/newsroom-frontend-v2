...
export default function AdminEntrevistas() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // 1️⃣ Solicitar Multipart Upload
      ...
      const { key, uploadId, partUrls, fileUrl, partSize } = await presignRes.json();

      // 2️⃣ Subir cada parte
      ...
      for (let i = 0; i < partUrls.length; i++) {
        ...
        if (!res.ok) throw new Error(`No se pudo subir la parte ${i + 1}`);

        // 🔑 ETag seguro, maneja comillas y minúsculas
        const etag =
          res.headers.get("ETag")?.replace(/"/g, "") ||
          res.headers.get("etag")?.replace(/"/g, "");
        if (!etag) throw new Error(`No se recibió ETag de la parte ${i + 1}`);
        etags.push({ ETag: etag, PartNumber: i + 1 });

        setProgreso(Math.round(((i + 1) / partUrls.length) * 100));
      }

      // 3️⃣ Completar Multipart Upload
      const completeRes = await fetch("/api/complete-multipart", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ key, uploadId, parts: etags }),
      });
      if (!completeRes.ok) throw new Error("No se pudo completar la subida");

      // 4️⃣ Guardar entrevista en backend
      ...
      if (!saveRes.ok) throw new Error("Error guardando entrevista");

      // ✅ Reset y mensaje de éxito
      setMensaje("✅ Entrevista subida correctamente");
      setTituloES(""); setTituloEN(""); setDescripcionES(""); setDescripcionEN("");
      setFecha(""); setVideoFile(null); setVideoPreview(null); setProgreso(0);
  ...
  return (
    <div className="min-h-screen p-8 bg-gray-50">
      ...
      >
        ...
        {videoFile && <p className="text-sm mt-1">Seleccionado: {videoFile.name}</p>}
        {videoPreview && (
          <video src={videoPreview} controls className="mt-2 w-full max-h-96 rounded-lg border"/>
        )}
        {subiendo && (
          <div className="w-full bg-gray-200 rounded-full h-4 mt-4 overflow-hidden">
            <div className="bg-blue-600 h-4" style={{ width: `${progreso}%` }}/>
          </div>
        )}
        <button
          type="submit"
